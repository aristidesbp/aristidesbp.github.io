...
``` 
-- =======================================================================================
-- 1. CORREÇÃO DE SEGURANÇA NO STORAGE DE FOTOS
-- =======================================================================================
-- Remove a política antiga que era permissiva
DROP POLICY IF EXISTS "Apenas autenticados sobem fotos" ON storage.objects;

-- Cria a política blindada que obriga a ser imagem e ter menos de 2MB (2097152 bytes)
...
-- =======================================================================================
-- 2. CORREÇÃO DAS RPCs DE PRODUTOS (MUDANDO DE DEFINER PARA INVOKER)
-- =======================================================================================
...
RETURNS uuid
LANGUAGE plpgsql
SECURITY INVOKER -- Substituído: Agora respeita o RLS
AS $$
DECLARE
    v_novo_id uuid;
BEGIN
...
-- =======================================================================================
-- 3. CORREÇÃO DA REGRA DE NEGÓCIO DA IMPORTAÇÃO NFe (Lógica cravada no Banco)
-- =======================================================================================
CREATE OR REPLACE FUNCTION public.inserir_produtos_lote_seguro(p_itens jsonb)
RETURNS void 
LANGUAGE plpgsql 
...
BEGIN
    ...
    LOOP
        ...
        -- MURALHA DE SEGURANÇA: Se o frontend enviar um preço de venda manipulado,
        -- o banco de dados recalcula e força a margem de 30% como valor mínimo.
...
-- =======================================================================================
-- 1. LIMPEZA TOTAL DAS POLÍTICAS ANTIGAS (Evita conflitos de segurança)
-- =======================================================================================
DROP POLICY IF EXISTS "Apenas autenticados leem produtos ativos" ON public.produtos;
DROP POLICY IF EXISTS "Apenas autenticados inserem produtos" ON public.produtos;
DROP POLICY IF EXISTS "Apenas autenticados atualizam produtos" ON public.produtos;
DROP POLICY IF EXISTS "Apenas autenticados deletam produtos" ON public.produtos;
DROP POLICY IF EXISTS "Leitura publica de produtos ativos" ON public.produtos;
DROP POLICY IF EXISTS "Apenas administradores atualizam produtos" ON public.produtos;

-- =======================================================================================
-- 2. RECRIAR POLÍTICAS RLS (NÍVEL BANCÁRIO)
-- =======================================================================================
-- Funcionários logados (authenticated) precisam de ver TUDO (incluindo a lixeira)
...
-- =======================================================================================
-- 3. GARANTIR DIREITOS DE EXECUÇÃO (GRANT)
-- =======================================================================================
-- Autoriza explicitamente os utilizadores logados a executarem as tuas RPCs
...
NOTIFY pgrst, 'reload schema';
``` 
# ajeitando duplicidade com xlm

``` 


-- =======================================================================================
-- ATUALIZAÇÃO DA RPC: IMPORTAÇÃO DE XML COM "UPSERT" (Soma de Estoque Automática)
-- =======================================================================================
CREATE OR REPLACE FUNCTION public.inserir_produtos_lote_seguro(p_itens jsonb)
RETURNS void 
LANGUAGE plpgsql 
...
BEGIN
    -- Inicia o loop para ler cada produto enviado pelo XML
    FOR item IN SELECT * FROM jsonb_array_elements(p_itens)
    LOOP
        -- Prepara as variáveis de preço com a tua regra de segurança bancária (Margem 30%)
        v_preco_custo := (item->>'preco_custo')::numeric;
        v_preco_venda := (item->>'preco_venda')::numeric;
        IF v_preco_venda < (v_preco_custo * 1.30) THEN
            v_preco_venda := v_preco_custo * 1.30;
        ...
        -- 1. VERIFICAÇÃO: Tenta encontrar o produto pelo EAN ou Código Interno
        -- Garantimos que não procuramos por campos em branco ('') para não misturar produtos vazios
        ...
        IF v_produto_existente_id IS NOT NULL THEN
            ...
            -- SITUAÇÃO A: O Produto já existe! Vamos atualizar o estoque e os custos.
            UPDATE public.produtos
            SET 
                -- Soma a quantidade antiga com a nova que chegou na NF-e
                quantidade = v_quantidade_atual + (item->>'quantidade')::numeric,
                -- Atualiza o custo e preço de venda para refletir a nova nota fiscal
        ...
        ELSE
            ...
            -- SITUAÇÃO B: É um produto totalmente novo. Vamos inserir.
            INSERT INTO public.produtos (
                nome, preco_custo, preco, codigo_produto, ean, ncm, cest, cfop, unidade_comercial, quantidade, valor_total, esta_ativo
            ) VALUES (
                item->>'nome', v_preco_custo, v_preco_venda, item->>'codigo_produto', item->>'ean', item->>'ncm', item->>'cest', 
        ...
        -- MUITO IMPORTANTE: Limpar a memória das variáveis para o próximo item do Loop não se confundir
...
-- Recarrega a API do Supabase para aplicar as alterações instantaneamente
NOTIFY pgrst, 'reload schema';
