...
import { uploadProjectToGoogleDrive } from '../../lib/googleDriveService';
import {
  Settings,
  Building2,
  Download,
...
export const SettingsView: React.FC = () => {
  ...
  const handleSaveToGoogleDrive = async () => {
    ...
    setDriveStatusMsg('Conectando à sua conta do Google e gerando o pacote ZIP do projeto...');
    setDriveFileUrl(null);

    const result = await uploadProjectToGoogleDrive();
    setIsSavingDrive(false);

    if (result.success) {
      setDriveStatusMsg(`✅ ${result.message}`);
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-500 text-white font-extrabold py-2.5 px-3 rounded-xl shadow-xs transition flex items-center justify-center gap-1.5 text-xs cursor-pointer"
                >
                ...
                <button
                  ...
                  className="bg-purple-600 hover:bg-purple-500 text-white font-extrabold py-2.5 px-3 rounded-xl shadow-xs transition flex items-center justify-center gap-1.5 text-xs cursor-pointer"
                >
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold py-2.5 px-3 rounded-xl shadow-xs transition flex items-center justify-center gap-1.5 text-xs cursor-pointer"
                >
          ...
          <div className="bg-gradient-to-br from-blue-900 to-indigo-950 text-white rounded-2xl p-6 shadow-md space-y-4 border border-blue-800/50">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-white text-base">
                    Salvar Projeto & Backup no Google Drive
                  </h3>
                  <p className="text-xs text-blue-200">
                    Sincronização direta de arquivo ZIP do projeto e banco de dados com a sua nuvem
                  </p>
                </div>
              </div>
              <span className="px-2.5 py-1 bg-blue-500/20 border border-blue-400/40 rounded-full text-[10px] font-black uppercase text-blue-200 tracking-wider">
                Google Workspace
              </span>
            </div>

            <p className="text-xs text-blue-100/90 leading-relaxed">
              Clique no botão abaixo para autenticar sua conta Google e salvar instantaneamente um arquivo <strong>.zip</strong> contendo todos os dados, configurações, produtos e vendas diretamente no seu Google Drive.
            </p>

            {driveStatusMsg && (
              <div className={`p-3 rounded-xl text-xs font-bold border ${driveStatusMsg.startsWith('✅') ? 'bg-emerald-950/80 border-emerald-500 text-emerald-200' : driveStatusMsg.startsWith('❌') ? 'bg-rose-950/80 border-rose-500 text-rose-200' : 'bg-blue-900/90 border-blue-500 text-blue-100'}`}>
                <p>{driveStatusMsg}</p>
                {driveFileUrl && (
                  <a
                    href={driveFileUrl}
                    ...
                    className="inline-flex items-center gap-1 mt-2 text-xs font-extrabold text-blue-300 hover:text-white underline"
                  >
                    <span>Abrir arquivo no Google Drive</span>
            ...
            <button
              ...
              className="w-full bg-white hover:bg-blue-50 text-blue-950 font-black py-3 px-4 rounded-xl shadow-lg transition flex items-center justify-center gap-2 text-xs cursor-pointer disabled:opacity-50"
            >
              <Cloud className={`w-4 h-4 ${isSavingDrive ? 'animate-bounce' : ''}`} />
              <span>{isSavingDrive ? 'Enviando ao Google Drive...' : 'Salvar no Meu Google Drive Agora'}</span>
            </button>
          </div>
