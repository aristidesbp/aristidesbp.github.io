...
export const EcommerceView: React.FC = () => {
  ...
  const activeProducts = allEcosystemProducts || products;
  ...
  const categories = Array.from(new Set(activeProducts.map((p) => p.categoria || 'Geral')));

  const availableProducts = activeProducts.filter((p) => p.quantidade_estoque > 0);
