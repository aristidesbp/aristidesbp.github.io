...
export const CustomerChatView: React.FC = () => {
  ...
  // Auto-sync all entities registered in the tenant account into WhatsApp CRM contacts
  useEffect(() => {
    if (entities && entities.length > 0) {
      setChats((prevChats) => {
        const existingPhones = new Set(
          prevChats.map((c) => (c.customer_phone ? c.customer_phone.replace(/\D/g, '') : ''))
        );
        const existingNames = new Set(prevChats.map((c) => c.customer_name.toLowerCase()));

        const newChatsFromEntities: CustomerChat[] = [];

        entities.forEach((ent) => {
          const rawPhone = ent.telefone || '(11) 99000-0000';
          const cleanPhone = rawPhone.replace(/\D/g, '');
          const typeLabel = ent.tipo_entidade === 'cliente' ? 'Cliente' : ent.tipo_entidade === 'fornecedor' ? 'Fornecedor' : 'Colaborador';
          const formattedName = `${ent.nome_completo} (${typeLabel})`;

          if (
            (!cleanPhone || !existingPhones.has(cleanPhone)) &&
            !existingNames.has(formattedName.toLowerCase()) &&
            !existingNames.has(ent.nome_completo.toLowerCase())
          ) {
            if (cleanPhone) existingPhones.add(cleanPhone);
            existingNames.add(formattedName.toLowerCase());

            newChatsFromEntities.push({
              id: `chat_ent_${ent.id}`,
              customer_name: formattedName,
              customer_phone: rawPhone,
              customer_avatar: ent.avatar_url || ent.foto_url,
              last_message: `Entidade cadastrada no ERP (${typeLabel})`,
              last_message_time: 'Hoje',
              unread_count: 0,
              status: 'online',
              ai_enabled: true,
              messages: [
                {
                  id: `msg_init_${ent.id}`,
                  chat_id: `chat_ent_${ent.id}`,
                  sender: 'store',
                  sender_name: storeConfig.store_name || 'Atendimento WhatsApp',
                  text: `Olá ${ent.nome_completo}! Contato registrado no ERP SaaS (${typeLabel}). Como podemos ajudar?`,
        ...
        if (newChatsFromEntities.length > 0) {
          return [...prevChats, ...newChatsFromEntities];
        }
        return prevChats;
      });
    }
