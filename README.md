    ...
    <script>
        ...
        async function gerarLancamentoCompleto() {
            ...
            try {
                ...
                if (editandoId) {
                    // Modo Edição (Atualiza apenas a parcela específica e o cabeçalho base)
        ...
        async function loadParcelas() {
            const { data } = await _supabase.from('parcelas').select('*, financas(descricao, tipo, categoria)').order('data_vencimento', { ascending: true });
            if(!data) return;
        ...
        async function prepararEdicao(id) {
            ...
            if (p) {
                ...
                document.getElementById('f-parcelas').disabled = true; // Impede alterar qtd na edição de uma parcela
