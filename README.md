...
"ERP_ABP", um marketplace de serviços comunitário, onde pessoas de
...
<style id="custom-styles">
    ...
    /* Inputs internos aos módulos fin/pdv (scoped para não sobrescrever o resto) */
...
<body class="bg-[#F8FAFC] dark:bg-[#0f172a] text-on-background dark:text-slate-200 font-body-md text-body-md overflow-x-hidden transition-colors duration-300">
 ...
 <!-- CONTAINER ONDE A NAVBAR SERÁ INJETADA -->
    <div id="navbar-container"></div>

    <!-- CONTEÚDO DA PÁGINA -->
    <main class="pt-24 px-4 sm:px-8">
        <h1 class="text-2xl font-bold">Painel Inicial (Home)</h1>
        <p>Bem-vindo ao sistema modularizado!</p>
    </main>

    <!-- IMPORTAÇÃO DO SCRIPT DA NAVBAR -->
    <script src="navbar.js"></script>

    <!-- INICIALIZAÇÃO ESPECÍFICA DA PÁGINA -->
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            // Renderiza a navbar definindo o valor ativo no <select> (ex: 'home')
            renderizarNavbar('home');
        });
    </script>
  ...
  <div class="card mb-8 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-premium space-y-6">
    <div>
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400">
        Gerencie as permissões do dispositivo, caminhos de armazenamento, backups no Supabase e importação de XML.
      </p>
    </div>
    ...
    <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onclick="cfg_solicitarPermissaoCamera()" class="bg-primary text-white px-6 py-3 rounded-xl font-bold hover:brightness-105 transition shadow-lg shadow-primary/10 flex items-center gap-2">
    ...
    <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onclick="cfg_solicitarPermissaoContatos()" class="bg-primary text-white px-6 py-3 rounded-xl font-bold hover:brightness-105 transition shadow-lg shadow-primary/10 flex items-center gap-2">
    ...
    <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <button onclick="cfg_selecionarPastaDownload()" class="bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-white px-5 py-3 rounded-xl font-bold hover:bg-slate-300 dark:hover:bg-slate-600 transition flex items-center gap-2 justify-center">
          <span class="material-symbols-outlined">folder_open</span> Escolher Pasta
        </button>
      </div>
      <div class="flex justify-end">
        <button onclick="cfg_salvarConfiguracaoPasta()" class="bg-primary text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:brightness-105 transition shadow-md shadow-primary/10">
          Salvar Caminho
        </button>
      </div>
    </div>

    <!-- 4. Backup e Restauração (Supabase) -->
    <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
      <div>
        <h4 class="font-bold text-slate-800 dark:text-white text-base">Backup e Restauração de Dados (Supabase)</h4>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Exporte uma cópia de segurança em formato JSON de todos os seus dados do sistema ou restaure a partir de um arquivo.</p>
      </div>
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <input type="file" id="input-importar-backup" accept=".json" class="hidden" onchange="cfg_restaurarBackupBanco(this)">
    ...
    <div class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 space-y-4">
      <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Selecione um arquivo XML de Nota Fiscal para cadastrar automaticamente produtos, fornecedor e lançamentos no financeiro via Supabase.
        </p>
      </div>
      <div>
        <input type="file" id="config-import-xml-nfe" accept=".xml" class="hidden" onchange="cfg_processarXmlNfeGeral(this)">
...
<div class="hidden fade-in max-w-7xl mx-auto px-4" id="aba-pdv">
  ...
  <div class="flex gap-4 mb-6">
    ...
    <button onclick="pdv_alternarSubAba('sangria')" id="pdv-btn-sangria" class="flex-1 bg-slate-200 text-slate-700 hover:bg-slate-300 font-bold py-3 rounded transition shadow">
      <i class="fas fa-money-bill-wave"></i> Sangria / Caixa Diário
    </button>
    <button onclick="pdv_alternarSubAba('historico')" id="pdv-btn-historico" class="flex-1 bg-slate-200 text-slate-700 hover:bg-slate-300 font-bold py-3 rounded transition shadow">
    ...
    <div class="lg:col-span-2 space-y-6">
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div class="md:col-span-2">
            <label class="block text-sm font-medium mb-1">Pesquisar ou Selecionar Produto</label>
            <input type="text" id="pdv_buscaProduto" placeholder="Digite o nome ou código..." list="listaProdutosDatalist" class="w-full p-2.5 border rounded dark:bg-slate-800 dark:border-slate-700 dark:text-white">
            <datalist id="listaProdutosDatalist"></datalist>
            <button type="button" onclick="pdv_adicionarDoInputBusca()" class="w-full mt-2 bg-slate-800 text-white text-sm font-bold py-2 rounded hover:bg-slate-700 transition">
      ...
      <div class="card sticky top-24 bg-slate-900 text-white">
        ...
        <div class="space-y-4">
          <div>
            ...
            <select id="pdv-cliente" class="w-full p-2 rounded bg-slate-800 border border-slate-700 text-white">
              <option value="">Consumidor Final (Não identificado)</option>
            </select>
          </div>
          <div>
            <label class="text-slate-300">Desconto Total (R$)</label>
            <input type="number" id="pdv-desconto" value="0.00" step="0.01" min="0" oninput="pdv_recalcularTotais()" class="w-full p-2 rounded bg-slate-800 border border-slate-700 text-white">
          </div>
          <div>
            <label class="text-slate-300">Forma de Pagamento *</label>
            <select id="pdv-forma-pagamento" class="w-full p-2 rounded bg-slate-800 border border-slate-700 text-white font-bold text-emerald-400">
  ...
  <div class="space-y-6 hidden" id="pdv-painel-sangria">
    <div class="card">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div><label class="block text-sm mb-1">Valor da Retirada (R$) *</label><input type="number" id="pdv-sangria-valor" step="0.01" placeholder="0.00" class="w-full p-2 border rounded dark:bg-slate-800 dark:border-slate-700"></div>
        <div class="md:col-span-2"><label class="block text-sm mb-1">Motivo / Justificativa *</label><input type="text" id="pdv-sangria-motivo" placeholder="Ex: Sangria para depósito bancário" class="w-full p-2 border rounded dark:bg-slate-800 dark:border-slate-700"></div>
      </div>
  ...
  <div class="space-y-6 hidden" id="pdv-painel-historico">
    <div class="card">
      <div class="flex justify-between items-center mb-4">
        <h3 class="font-bold text-lg text-slate-800 dark:text-white"><i class="fas fa-list-alt text-emerald-500"></i> Histórico Geral de Vendas</h3>
        <button onclick="pdv_loadHistoricoVendas()" class="bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300 px-3 py-1.5 rounded text-sm font-bold hover:bg-slate-300 transition">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody id="pdv-lista-vendas-historico" class="text-sm dark:text-slate-300"></tbody>
        </table>
      </div>
    </div>
  </div>
...
<div id="pdv-cupom-impressao" class="hidden">
  <div style="font-family: monospace; width: 300px; padding: 10px; margin: 0 auto; color: #000; background: #fff;">
    ...
    <p style="font-size: 11px; margin: 2px 0;">Data: <span id="cupom-data"></span></p>
    <p style="font-size: 11px; margin: 2px 0;">ID Venda: <span id="cupom-id"></span></p>
    ...
    <table style="width: 100%; font-size: 11px;">
      ...
      <tbody id="cupom-itens-corpo"></tbody>
    </table>
    <hr style="border: dashed 1px #000; margin: 8px 0;">
    <div style="font-size: 12px; display: flex; justify-content: space-between;"><span>Subtotal:</span> <span id="cupom-subtotal"></span></div>
    <div style="font-size: 12px; display: flex; justify-content: space-between;"><span>Desconto:</span> <span id="cupom-desconto"></span></div>
    <div style="font-size: 14px; font-weight: bold; display: flex; justify-content: space-between; margin-top: 5px;"><span>TOTAL:</span> <span id="cupom-total"></span></div>
