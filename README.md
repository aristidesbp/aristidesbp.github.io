...
create table public.servicos (
  ...
  preco_estimado double precision null,
  preco_detalhe text null,
  foto_url text null,
  criado_por bigint null,
  terceiro boolean null default false,
  eu_mesmo boolean null default true,
  whatsap text null,
  constraint servicos_pkey primary key (id),
  constraint servicos_categoria_fkey foreign KEY (categoria) references categorias (id),
  constraint servicos_criado_por_fkey foreign KEY (criado_por) references usuarios (id)
) TABLESPACE pg_default;
...
create table public.favoritos (
  ...
  constraint favoritos_servico_id_fkey foreign KEY (servico_id) references servicos (id),
  constraint favoritos_usuario_id_fkey foreign KEY (usuario_id) references usuarios (id)
) TABLESPACE pg_default;
...
deve ser criado o mesmo usuario no public na tabela abaixo.
...
CREATE TABLE public.servicos (
  ...
  CONSTRAINT servicos_criado_por_fkey FOREIGN KEY (criado_por) REFERENCES public.usuarios(id)
);
CREATE TABLE public.usuarios (
  id bigint GENERATED ALWAYS AS IDENTITY NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
...
## CRIANDO SEGURANÇA:
```
create or replace view public.v_servicos_destaque with (security_invoker) as
select 
    -- Detalhes do Serviço
    s.id as servico_id,
    s.titulo,
