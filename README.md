...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.entidades (user_id, nome_completo, email, tipo_acesso, tipo_entidade, status_entidade)
  VALUES (
    NEW.id, -- O ID que vem do auth.users
    COALESCE(NEW.raw_user_meta_data->>'full_name', 'Novo Utilizador'), -- Tenta apanhar o nome dos metadados, se não houver usa um padrão
    NEW.email, -- O email do auth.users
    'cliente', -- Valor padrão para tipo_acesso
    'cliente', -- Valor padrão para tipo_entidade
    'ativo'    -- Valor padrão para status_entidade
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 2. Criar o gatilho (Trigger) que chama a função após um insert em auth.users
