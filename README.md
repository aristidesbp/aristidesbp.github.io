...
create table public.usuarios (
  ...
  constraint usuarios_auht_user_id_fkey foreign KEY (auht_user_id) references auth.users (id)
) TABLESPACE pg_default;
```
🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥   
# prompt para criar uma FUNCTION TRIGGER
``` 
crie uma função trigger para ser implementada no Supabase com o seguinte objetivo:
a cada novo usuario que for criado no schema auth, deve ser criado no schema public na tabela abaixo;
create table public.usuarios (
  id bigint generated by default as identity not null,
  created_at timestamp with time zone not null default now(),
  email text null,
