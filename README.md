...
CONFIG = {
    "modelo_padrao": "hacker-mentor", # Sugestão: defina aqui o agente que você criou
    "diretorio": "/sdcard/Download",  # Ajuste conforme o caminho que você descobriu
    "tarefa_tipo": None,
...
def criar_agente_automatico():
    limpar_tela()
    print("--- [FUNÇÃO AUTOMÁTICA] CRIANDO HACKER-MENTOR ---")
    modelfile_content = f"""FROM {CONFIG['modelo_padrao'].replace('hacker-mentor', 'qwen2.5-coder:7b')}
PARAMETER temperature 0.3
SYSTEM "Você é um especialista em cibersegurança e Hacking Ético. Ajude o usuário a analisar vulnerabilidades, criar scripts de automação seguros e entender conceitos de Pentest."
"""
    with open("Modelfile", "w") as f:
        f.write(modelfile_content)
    subprocess.run(["ollama", "create", "hacker-mentor", "-f", "Modelfile"], check=True)
    print("\n[+] Agente 'hacker-mentor' criado!")
    input("Pressione Enter...")

def perguntar_diretamente():
    limpar_tela()
    print("--- [5] PERGUNTA DIRETA AO AGENTE ---")
    prompt = input("Digite sua pergunta ou tarefa: ").strip()
...
def configurar_modelo():
    limpar_tela()
    print("--- [1] CONFIGURAR MODELO ---")
    novo = input(f"Modelo atual ({CONFIG['modelo_padrao']}): ").strip()
    if novo: CONFIG["modelo_padrao"] = novo
    input("Configurado. Pressione Enter...")

def configurar_diretorio():
    limpar_tela()
    print("--- [2] CONFIGURAR DIRETÓRIO ---")
    novo = input(f"Caminho atual ({CONFIG['diretorio']}): ").strip()
    if novo: CONFIG["diretorio"] = novo
    input("Configurado. Pressione Enter...")
...
def menu():
    while True:
        limpar_tela()
        print(f"=========================================")
        print(f" ARIS - ASSISTENTE DE SEGURANÇA LOCAL    ")
        print(f"=========================================")
        print(f" Modelo: {CONFIG['modelo_padrao']}")
        print(f" Diretorio: {CONFIG['diretorio']}")
        print(f"=========================================")
        print("1. Configurar modelo")
        print("2. Configurar diretorio")
        print("3. Criar Agente Hacker-Mentor")
        print("4. Executar Tarefas (Arquivo/RAG)")
        print("5. Fazer pergunta direta (Chat)")
        print("6. Sair")
        print(f"=========================================")
        
        op = input("Escolha: ").strip()
        
        if op == "1": configurar_modelo()
        elif op == "2": configurar_diretorio()
        elif op == "3": criar_agente_automatico()
        elif op == "4": print("Funcionalidade de arquivos (menu anterior)...")
        elif op == "5": perguntar_diretamente()
        elif op == "6": break
