...
* 🌐 **Login:** [ENTRAR](https://aristidesbp.github.io/projetos/login.html)
...
```
# criar tabela esplho do user_auth:
```
...
# prompt para criar functions triger

```
Cria uma função trigger para ser implementada no Supabase co o seginte objetivo:

1- A cada novo usuário que for criado no schema auth, deve ser criado o mesmo usuário no schema public na tabela abaixo:
...
## 1. Criar a função que realiza a inserção

A função abaixo utiliza o NEW (que representa o registro recém-criado no auth.users).
...
as $$
begin
  insert into public.usuarios (user_id_auth, role)
  values (new.id, 'cliente'); -- Definimos 'cliente' como padrão inicial
  return new;
end;
$$;
```
...
Vinculamos o evento de inserção na tabela auth.users à nossa função:
