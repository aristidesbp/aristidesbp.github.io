...
<div class="md:col-span-4 mt-2">
  ...
  <div class="flex gap-2">
    <input type="text" id="f-codigo-barras" placeholder="Cole ou leia o código de barras da carteirinha" class="flex-1 p-2.5 border rounded-lg bg-slate-50 dark:bg-slate-800 dark:text-white dark:border-slate-700">
    <button onclick="ent_iniciarLeituraCamera()" type="button" class="bg-slate-800 text-white px-4 rounded-lg hover:bg-slate-700 transition flex items-center gap-2">
      <i class="fas fa-camera"></i> Ler Código
    </button>
  </div>
  <div id="ent-camera-container" class="hidden mt-3 relative border-2 border-dashed border-slate-300 p-2 rounded bg-slate-50 dark:bg-slate-800">
    <div id="ent-camera-preview" class="w-full max-w-sm mx-auto overflow-hidden rounded"></div>
    <button onclick="ent_pararCamera()" type="button" class="absolute top-4 right-4 bg-red-500 text-white w-8 h-8 rounded-full flex justify-center items-center hover:bg-red-600 shadow-lg z-10">
...
async function salvarEntidade() {
    ...
    try {
        ...
        const payload = {
            nome_completo: nome, cpf, data_nascimento: nascimento, email, telefone,
            tipo_entidade: tipo, status_entidade: status, cep, logradouro, numero,
            bairro, cidade: city, estado: state, codigo_barras: codigoBarras || null, user_id: usuarioLogadoId
        };
...
// ── Controle do Leitor de Código de Barras via Câmera ─────────────────
let html5QrCodeScanner = null;

function ent_iniciarLeituraCamera() {
    const container = document.getElementById('ent-camera-container');
    container.classList.remove('hidden');

    if (!html5QrCodeScanner) {
        html5QrCodeScanner = new Html5Qrcode("ent-camera-preview");
    }
    ...
    html5QrCodeScanner.start(
        ...
        config,
        (decodedText) => {
            document.getElementById('f-codigo-barras').value = decodedText;
            ent_pararCamera();
        },
...
function ent_pararCamera() {
    if (html5QrCodeScanner && html5QrCodeScanner.isScanning) {
        html5QrCodeScanner.stop().then(() => {
            document.getElementById('ent-camera-container').classList.add('hidden');
