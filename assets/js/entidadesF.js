        ...
        async function loadEntidades() {
            ...
            tbody.innerHTML = data.map(e => {
                ...
                return `
                <tr class="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td class="px-6 sm:px-8 py-5 text-center"><input type="checkbox" class="check-entidade w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-primary focus:ring-primary bg-white dark:bg-slate-800" value="${e.id}"></td>
                    <td class="px-6 sm:px-8 py-5">
                        <span class="bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-lg font-bold text-[10px] uppercase text-slate-500 dark:text-slate-400 group-hover:bg-primary group-hover:text-white transition-all">
                            ${e.tipo_entidade}
                        </span>
                    </td>
                    <td class="px-6 sm:px-8 py-5">
                        <div class="flex items-center gap-4">
                            <div class="relative shrink-0">
                                <img src="${imgUrl}" class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border-2 border-white dark:border-slate-700 shadow-sm object-cover bg-slate-100 dark:bg-slate-800">
                                <div class="absolute -bottom-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 border-white dark:border-slate-800 ${e.status_entidade === 'ativo' ? 'bg-emerald-500' : 'bg-slate-300'}"></div>
                            </div>
                            <div class="min-w-0">
                                <div class="font-bold text-slate-900 dark:text-white truncate max-w-[150px] sm:max-w-none">${e.nome_completo}</div>
                                <div class="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5 truncate">
                                    <span class="material-symbols-outlined text-[14px]">mail</span> ${e.email || 'N/A'}
                                </div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 sm:px-8 py-5 font-mono-sm text-sm text-slate-500 dark:text-slate-400">${e.cpf || '---'}</td>
                    <td class="px-6 sm:px-8 py-5 text-sm text-slate-600 dark:text-slate-400">
                        <div class="font-bold text-slate-800 dark:text-slate-300 truncate max-w-[100px] sm:max-w-none">${e.cidade || '---'}</div>
                        <div class="text-[11px] text-slate-400 uppercase tracking-wider">${e.estado || '--'}</div>
                    </td>
                    <td class="px-6 sm:px-8 py-5 text-center"><span class="${statusClass}">${e.status_entidade}</span></td>
                    <td class="px-6 sm:px-8 py-5 text-right">
                        <button onclick="prepararEdicao('${e.id}')" class="text-slate-400 hover:text-primary hover:bg-primary/5 p-2 rounded-xl transition-all active:scale-90">
        ...
        async function excluirSelecionados() {
            ...
            if(confirm(`Confirmar exclusão de ${selecionados.length} registros?`)) {
                ...
                if(!error) { alert("Registros excluídos."); init(); }
