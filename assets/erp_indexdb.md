...
async function est_processarXML(input) {
    ...
    if (btnImport.innerHTML !== undefined) { btnImport.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Lendo XML...'; btnImport.disabled = true; }
    ...
    reader.onload = async function(e) {
        try {
            ...
            for (let i=0; i<itens.length; i++) {
                ...
                const existente = produtosAtuais.find(p => (ean && p.codigo_barras === ean) || p.nome === nome);

                if (existente) {
                    existente.quantidade_estoque = parseInt(existente.quantidade_estoque||0) + qtd;
                    ...
                    produtosAtuais.push(novoProd);
                    novos++;
                }
            }
            alert(`NF-e importada!\n✅ Novos: ${novos}\n🔄 Atualizados: ${atualizados}`);
            input.value='';
            est_loadProdutos(); est_loadDashboard();
        } catch(err) {
            alert("Erro ao processar XML: "+err.message);
        } finally {
            if (btnImport.innerHTML !== undefined) { btnImport.innerHTML = textoOriginal; btnImport.disabled = false; }
        }
    };
